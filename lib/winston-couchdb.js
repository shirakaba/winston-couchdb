...
module.exports = class Couchdb extends TransportStream {
  ...
  _ensureView(callback = (() => {})) {
    ...
    const checkDB = () => {
      // TODO: This syntax might not be in Nano. Fix it
      this.client.exists( (err, exists) => {
  ...
  query(options, callback) {
    ...
    // TODO: This syntax might be wrong; fix it
    this.client.view('Logs/byTimestamp', query, (err, docs) => {
      if (err) return callback(err);
  ...
  stream(options) {
    ...
    this.client.info((err, info) => {
      ...
      // TODO: verify syntax is correct
      // Possibly allow some kind
      // of basic querying here?
      feed = this.client.changes({
        include_docs: true,
        feed: 'continuous',
        style: 'main_only',
        descending: false,
