 ...
 */
class Couchdb extends TransportStream {
  /**
   ...
   * @param {Object} msg Message to log with meta and message
   * @param {function} callback Continuation to respond to when complete.
   */
  log(msg, callback) {
    if (typeof msg === Object.name.toLowerCase()) {
      return this.legacylog(msg.level || 'info', msg.message, msg.meta || {}, callback);
    }

    // This is to account for the legacy logger
    const args = [...arguments];
    let [level, message, meta, cb] = args;

    return this.legacylog(level || this.level || 'info', message, meta, cb);
  }
   ...
   * @param {function} callback Continuation to respond to when complete.
   */
  legacylog(level, msg, meta, callback) {
    if (this.silent) {
   ...
   */
  static registerTransport(_winston = winston) {
    /**
     * TODO: add property description.
     * @type {Couchdb}
     */
    Object.defineProperty(_winston.transports, 'Couchdb', {
      configurable: true,
      enumerable: true,
      get() {
        return Couchdb;
